...
def kb_for_invoices(inv_numbers: list[str], date: str) -> dict:
    rows = [
        [{"text": n, "callback_data": f"del_inv|{date}|{n}"}] for n in inv_numbers
    ]
...
async def handle_delete_callback(cq: dict):
    ...
    if action == "del_date":
        ...
        if not invs:
            ...
            return
        await edit(f"Date: <b>{date_iso[:10]}</b>\nSelect invoice number:",
                   kb_for_invoices(invs, date_iso))
        return
    ...
    if action == "del_inv":
        ...
        if not items:
            ...
            return
        await edit(f"Invoice <b>{inv}</b>\nSelect item to delete:",
                   kb_for_items(items, date_iso, inv))
        return

    if action == "del_item":
        _, inv, item = parts                     # date is not needed for deletion
...
Ready to get started? Just tell me about your first sale or ask me anything!
        ...
        # ───────────── callback queries section ─────────────
        if "callback_query" in update:
            cq       = update["callback_query"]
            chat_id  = cq["message"]["chat"]["id"]
            payload  = cq["data"]

            # Helper to edit the existing message instead of sending new ones
            async def edit(msg_id: int, new_text: str, kb: dict | None = None):
                async with httpx.AsyncClient(timeout=10) as client:
                    await client.post(
            ...
            # -------- 1) user picked a date --------
            if action == "del_date":
                date = parts[0]
                inv_numbers = get_invoice_numbers(chat_id, date)
                if not inv_numbers:
                    await send("No invoices found for that date.")
                    return "OK"
                kb = kb_for_invoices(inv_numbers, date)
                await edit(cq["message"]["message_id"],
                           f"Date: <b>{date}</b>\nSelect invoice number:",
                           kb)
                return "OK"

            # -------- 2) user picked an invoice number --------
            if action == "del_inv":
                date, inv = parts
                items = get_item_names(chat_id, inv)
                if not items:
                    await send("No items under that invoice.")
                    return "OK"
                kb = kb_for_items(items, date, inv)
                await edit(cq["message"]["message_id"],
                           f"Invoice <b>{inv}</b>\nSelect item to delete:",
                           kb)
                return "OK"

            # -------- 3) user picked an item --------
            if action == "del_item":
                date, inv, item = parts
                ok = delete_transaction(chat_id, inv, item)
                if ok:
                    await edit(
                        ...
                        f"✅ Deleted <b>{item}</b> from invoice {inv} ({date})."
                    )
                else:
                    await edit(
                        cq["message"]["message_id"],
                        "❌ Nothing deleted (item might have already been removed)."
                    )
                return "OK"

            # -------- 4) cancel at any level --------
            if action == "del_cancel":
                await edit(
                    cq["message"]["message_id"],
